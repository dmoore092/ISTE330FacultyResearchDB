	...
	*/
	...
	function searchProjects($name){
		try{
			$name = "%".$name."%";
                	$data = array();
               		$stmt = $this->dbConn->prepare("select projectName, projectLead, email, projectDescription from project JOIN user WHERE project.name = user.name AND projectName LIKE :name"); 
