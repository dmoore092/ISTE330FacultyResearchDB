	...
	*/
	...
	function searchProjects($name){
		try{
			$name = "%".$name."%";
                	$data = array();
               		$stmt = $this->dbConn->prepare("select projectName, name as 'projectLead', email, projectDescription from project JOIN user WHERE project.projectLead = user.id AND projectName LIKE :name"); 
