	...
	*/
	...
	function searchProjects($name){
		try{
			$name = "%".$name."%";
                	$data = array();
               		$stmt = $this->dbConn->prepare("select projectName, projectLead, email, projectDescription from project JOIN user WHERE project.projectLead = user.name AND projectName LIKE :name"); 
