	...
	*/
	...
	function searchProjects($name){
		try{
			$name = "%".$name."%";
                	$data = array();
               		$stmt = $this->dbConn->prepare("SELECT projectName, name AS 'projectLead', email, projectDescription FROM project JOIN user ON project.projectLead = user.id WHERE projectName LIKE :name"); 
